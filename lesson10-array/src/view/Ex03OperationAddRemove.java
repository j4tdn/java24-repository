...
public class Ex03OperationAddRemove {
	...
	private static String[] add(String[] source, int pos, String newValue) {
		if (pos < 0 || pos > source.length) {
			throw new IllegalArgumentException("Vị trí thêm thuộc ngoài phạm vi của mảng");
		}
	...
	private static String[] remove(String[] source, int pos) {
		if (pos < 0 || pos >= source.length) {
			throw new IllegalArgumentException("Vị trí xóa thuộc ngoài phạm vi của mảng");
		}
