...
public class Ex03Operation {
	...
	private static String[] add(String[] source, int pos, String newValue) {
		...
		/*
		 * // B1: Tạo mảng mới gồm n+1 phần tử String[] target = new
		 * String[source.length + 1];
		 * 
		 * // B2: Copy các phần tử từ source sang target đúng vị trí for (int i = 0; i <
		 * source.length; i++) { target[i] = source[i]; }
		 * 
		 * // B3: Dịch phải 1 đơn vị từ length-1 về phần tử > pos for (int i =
		 * target.length-1; i > pos; i--) { target[i] = target[i-1]; }
		 * 
		 * // B4: Gán phần tử mới vào pos target[pos] = newValue;
		 */
