...
public static <E, K, V> Map<K, V> filterToMap(Collection<E> collection, Predicate<E> predicate,
	...
	return collection.stream().filter(t -> predicate.test(t))
			.collect(Collectors.toMap(k -> keyMapper.apply(k), v -> valMapper.apply(v)));
}

public static <E, K, V> Map<K, V> filterToMap(Collection<E> collection, Function<E, K> keyMapper,
